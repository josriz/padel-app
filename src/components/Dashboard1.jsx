// src/components/Dashboard.jsx - TUO FILE COMPLETO + FIX MINIMI
...
export default function Dashboard() {
  ...
  // ✅ FIX: Usa profiles invece di user_roles + role da AuthProvider
  useEffect(() => {
      ...
      if (user?.id && !role) {
        try {
          ...
          if (error && error.code !== 'PGRST116') { // PGRST116 = no rows
            console.log('No role found:', error.message);
          } else if (data?.role) {
  ...
  // ✅ Usa TUOI valori role da Supabase profiles
  const isTorneiAdmin = !roleLoading && ['admin', 'superadmin'].includes(userRole);
  const isMarketplaceAdmin = !roleLoading && ['admin', 'superadmin'].includes(userRole);
  const isSuperAdmin = !roleLoading && userRole === 'superadmin';
  ...
  if (roleLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-500 via-blue-500 to-purple-600">
        <div className="text-white text-2xl animate-pulse">Caricamento ruolo...</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="relative z-10 flex-1">
        ...
        {/* CARDS - IDENTICO con immagini fixate */}
